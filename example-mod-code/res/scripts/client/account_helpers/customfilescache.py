...
def _LOG_EXECUTING_TIME(startTime, methodName, deltaTime=0.1):
    ...
    if finishTime - startTime > deltaTime:
        LOG_WARNING(
            'Method "%s" takes too much time %s' % (methodName, finishTime - startTime)
        )
...
class CustomFilesCache(object):
            ...
            if file_hash in cache:
                ...
                if data is None:
                    ...
                    if expires is None:
                        ...
                        return
                    if (
                        ctime - _MIN_LIFE_TIME
                        <= expires
                        <= ctime + _MAX_LIFE_TIME + _MIN_LIFE_TIME
                    ):
                        LOG_DEBUG(
                            "postTask, Sends file to requester.",
                            url,
                            last_modified,
                            data[0],
                        )
                        ...
                        if showImmediately:
                            LOG_DEBUG(
                                "postTask, Do not release callbacks. Sends file to requester.",
                                url,
                                last_modified,
                                data[0],
                            )
                        ...
                        LOG_DEBUG(
                            "readRemoteFile, there is file in cache, check last_modified field.",
                            url,
                            last_modified,
                            data[0],
                        )
                        self.__readRemoteFile(
                            url, last_modified, showImmediately, headers
                        )
            elif checkedInCache:
    ...
    def __onReadLocalFile(self, url, showImmediately):
        ...
        try:
            remoteFile = (
                self.__db[key] if self.__db is not None and key in self.__db else None
            )
    ...
    def __readRemoteFile(self, url, modified_time, showImmediately, headers):
        task = {
            "opType": CFC_OP_TYPE.DOWNLOAD,
            "callback": partial(
                self.__onReadRemoteFile, url, showImmediately, modified_time, headers
            ),
        }
        self.__worker.add_task(task)
    ...
    def __prepareCache(self, recreate=False):
        try:
            ...
            try:
                self.__db = provider.open(
                    filename,
                    protocol=HIGHEST_PICKLE_PROTOCOL,
                    flag=flag,
                    writeback=True,
                )
            except UnpicklingError:
                LOG_WARNING(
                    "__prepareCache, unpickling with highest protocol has been failed, falling back to old protocol"
                )
    ...
    def __postTask(self, url, remote_file, invokeAndReleaseCallbacks):
        BigWorld.callback(
            0.001,
            partial(self.__onPostTask, url, invokeAndReleaseCallbacks, remote_file),
        )
